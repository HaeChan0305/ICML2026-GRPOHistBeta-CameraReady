...
We further benchmark DBB against three alternative shrinkage estimators: per-prompt exponential moving average (EMA), per-prompt Laplace smoothing, and an online empirical-Bayes (EB) variant that shares a Beta prior across prompts.
Appendix~\ref{ap:ema_laplace} shows that DBB attains the lowest minimum MSE and improves on the point estimator over a broader $\lambda$ range than either EMA or Laplace.
Appendix~\ref{ap:emp_bayes} further shows that EB, while sidestepping per-prompt $\lambda$ tuning, still attains a higher minimum MSE than DBB; combining the hierarchical prior with the discounted update is a natural direction for future work.
