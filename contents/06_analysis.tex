...
We attribute this pattern to how DBB handles zero-variance prompts---those for which all sampled rewards within a group are zero.
For such prompts, the group-relative advantage of GRPO collapses to zero and yields no training signal, whereas DBB still produces a non-zero (negative) advantage that rapidly suppresses incorrect trajectories.
This stronger pruning of zero-variance prompts removes unproductive token trajectories from the policy's tree, lowering entropy, and shifts exploration from the breadth-wise mode (diversifying trajectories) toward the depth-wise mode (extending them), naturally yielding the longer responses observed.
